...
class Attention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def __init__(
        ...
        multiplier_value = 2 if not self.variational else 3
        multiplier_value_out = 1 if not self.variational else 3
        self.linear_out = nn.Linear(
            dim * multiplier_value, dim * multiplier_value_out, bias=out_bias
        )
    ...
    def forward(
        self, source, memory_bank, memory_lengths=None, coverage=None, latent=None
    ):
        """
        ...
        out_c = c.view(batch, dim)

        # concatenate
        if latent is not None:
            concat_c = torch.cat([c, source, latent.unsqueeze(1)], 2).view(
                batch * target_l, dim * 3
            )
            attn_h = self.linear_out(concat_c).view(batch, target_l, 3 * dim)
        else:
        ...
        return attn_h, align_vectors, out_c
